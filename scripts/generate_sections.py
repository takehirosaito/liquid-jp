...
# Phase 6β 教訓 v1.1(必ず守る 8 ルール)

## ルール 1: 文体は「ですます調」で統一(マーチャント可視テキストのみ)
settings の default / heading / subheading / empty メッセージはすべて「ですます調」。
「〜する。」「〜される。」「〜できる。」「〜てほしい。」を使わない。
- NG: "を紹介する。" "を選択する。" "効果を実感できる。" "ぜひ一度試してみてほしい。"
- OK: "をご紹介します。" "をご選択ください。" "効果を実感できます。" "ぜひ一度お試しください。"
コメントや info(技術解説)のみ「だ・である調」可。

## ルール 2: 日付・時刻系の default は遠未来(2099 年)
NG: `"default": 2025`(本番投入時に全件「終了しました」になる)
OK: `"default": 2099`
year / end_year / start_year など年フィールドはすべて 2099 を default にする。

## ルール 3: blocks ありセクションは必ず空フォールバック実装
`{%- for block in section.blocks -%}` の直前に必ず空判定:
```liquid
...
- 実績系: 「表示できる実績はまだありません。」
- 商品系: 「表示する商品をテーマエディタから追加してください。」

## ルール 4: 画像 + テキスト 2 カラムは画像未設定時に 1 カラム化
メイン画像 + テキスト構成の場合、`--no-image` クラスで 1 カラム化:
```liquid
<section class="alsel-{slug}{%- if section.settings.main_image == blank %} alsel-{slug}--no-image{%- endif -%}" ...>
```
CSS で:
```css
.alsel-{slug}__inner { display: grid; grid-template-columns: 1fr 1fr; }
.alsel-{slug}--no-image .alsel-{slug}__media { display: none; }
.alsel-{slug}--no-image .alsel-{slug}__inner { grid-template-columns: 1fr; max-width: 800px; text-align: center; }
```

## ルール 5: 同種セクションは preset.name と default heading の両方で UI タイプを明示
- NG: `"name": "ALSEL監修 / お客様の声カード"` + `"default": "お客様の声"`
- OK: `"name": "ALSEL監修 / お客様の声(カードグリッド)"` + `"default": "お客様の声(カードグリッド)"`
命名規則: `ALSEL監修 / <用途>(<UI バリエーション or 形式>)`
- UI バリエーション: カードグリッド / 横スクロール / 表彰台型 TOP3 / アコーディオン / モーダル / スティッキー / グラデーション帯
- 形式: TOP3 / TOP5 / TOP10 / 2 カラム / 3 カラム / 4 カラム

## ルール 6: 短い 1 単語ラベルを単独で出さない
バッジ等の独立した短ラベルは前後関係なく意味が通るように補完。
- NG: 「期間」「特典」「新着」「人気」「限定」「特集」
- OK: 「期間限定」「特典あり」「新着商品」「人気商品」「期間限定」「特集企画」

## ルール 7: PC/モバイル切替系の default はガイド文に
本番文ではなく「ここに〜を入力します」のマーチャント向けガイド文を default にする。
- NG: `"default": "PC で見るキャッチコピー"`
- OK: `"default": "PC 表示時のメインメッセージ"`

## ルール 8: preset.blocks を必ず明示
blocks ありセクションは preset に最低 2〜3 個のデモ blocks を含める。
API 直投入時に「土台だけ表示」になる問題を防ぐ。
```json
"presets": [{
  "name": "ALSEL監修 / ...",
  "settings": { ... },
...
# 出力前の最終チェック(自己レビュー、必須)
1. 冒頭 {% comment %} ブロックがある(全項目記載)
2. <section class="alsel-{slug}" aria-label="..."> でラップされている
3. <style> でスコープ付き CSS(全 class が alsel-{slug}__ プレフィックス)
4. {% schema %} の name / settings / presets が完備
5. presets[0].name が "ALSEL監修 / {セクション名}(<バリエーション>)" 形式(同種があれば明示)
6. {% include %} を使っていない
7. すべての settings の label が日本語、default が「ですます調」
8. image_tag の引数内に `| escape` を書かない(alt は事前に assign で変数化)
9. font-family は指定しない(テーマ継承)、font-weight 最大 700
10. blocks ありセクションには空フォールバック + preset.blocks 2-3 個
11. 画像 + テキスト 2 カラム構成なら `--no-image` 切替実装
12. 年 default は 2099
13. 300 行以内
